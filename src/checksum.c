...
int check_data_packet_checksum(const void *packet, uint32_t secret)
{
	...
	// 	return 0;
	checksum32_data[0] = secret;
	checksum32_data[1] = (0xff00 & (_packet->line->record_type[0] << 8)) | (0xff & _packet->line->record_type[1]);
	checksum32_data[2] = _packet->line->byte_count;
	checksum32_data[3] = _packet->line->address;
	checksum32_data[4] = _packet->line->checksum;

	for (int i = 0; i < _packet->line->data.len; i++)
	{
		int helper = 3 - i % 4;
		checksum32_data[5 + i / 4] = checksum32_data[5 + i / 4] | (helper_mask[helper] & (_packet->line->data.data[i] << (8 * helper)));
	}

	CRC_ResetDR();
	checksum = CRC_CalcBlockCRC(checksum32_data, _packet->line->byte_count / 4 + 5);
